...
export default async function SessionsPage({
  ...
  type SessionListRow = Pick<
    SessionRow,
    "session_id" | "citizen_display_name" | "center_id" | "code" | "code_expires_at" | "created_at"
  >;

  // A center is required so this page never issues an unscoped, fleet-wide query -- without one
  // selected we skip the sessions query entirely and just show the filter form.
  let filtered: SessionListRow[] = [];
  if (center) {
    let query = supabase
      .from("sessions")
    ...
    <>
      ...
      <PageShell>
        <form method="get" className="flex flex-col gap-3">
          <Field label="Center">
            <Select name="center" defaultValue={center} required>
        ...
        {!center ? (
          <EmptyState>Select a center above to view its sessions.</EmptyState>
        ) : filtered.length === 0 ? (
