...
-- No document type is built-in -- every type and field is admin-managed via the dashboard, with no
-- hardcoded catalog anywhere in the app. `sensitive` (admin-set per field) drives masking in place
-- of the old hardcoded "is this the Aadhaar type's aadhaar_number field" check -- see
-- FieldMaskingRules.kt / GenericMasker.kt on the Android side.
